...
/**
 * Twitter "spritzer" Firehose Factory named "twitzer". Builds a Firehose that
 * emits a stream of ?? with timestamps along with ??. The generated tuples have
 * the form (timestamp, ????) where the timestamp is from the twitter event.
 * <p/>
 * Example spec file:
 * <p/>
 * Example query using POST to /druid/v2/?w (where w is an arbitrary parameter
 * and the date and time is UTC):
 * <p/>
 * Notes on twitter.com HTTP (REST) API: v1.0 will be disabled around 2013-03 so
 * v1.1 should be used; twitter4j 3.0 (not yet released) will support the v1.1
 * api. Specifically, we should be using
 * https://stream.twitter.com/1.1/statuses/sample.json See:
 * http://jira.twitter4j.org/browse/TFJ-186
 * <p/>
 * Notes on JSON parsing: as of twitter4j 2.2.x, the json parser has some bugs
 * (ex: Status.toString() can have number format exceptions), so it might be
 * necessary to extract raw json and process it separately. If so, set
 * twitter4.jsonStoreEnabled=true and look at DataObjectFactory#getRawJSON();
...
public class TwitterSpritzerFirehoseFactory implements FirehoseFactory<InputRowParser> {
	 ...
	 * this to prevent infinite space consumption or to prevent getting
	 * throttled at an inconvenient time or to see what happens when a Firehose
	 * stops delivering values, or to have hasMore() return false. The Twitter
	 * Spritzer can deliver about 1000 events per minute.
	 */
	private final int maxEventCount;
	/**
	 * maximum number of minutes to fetch Twitter events. Use this to prevent
	 * getting throttled at an inconvenient time. If zero or less, no time limit
	 * for run.
	 */
	private final int maxRunMinutes;

	@JsonCreator
	public TwitterSpritzerFirehoseFactory(@JsonProperty("maxEventCount") Integer maxEventCount,
	...
	public Firehose connect(InputRowParser parser) throws IOException {
		...
		/**
		 * This queue is used to move twitter events from the twitter4j thread
		...
		statusListener = new StatusListener() { // This is what really gets
												// called to deliver stuff from
												// twitter4j
			@Override
			...
			public void onTrackLimitationNotice(int numberOfLimitedStatuses) {
				...
				// unlimited.
				// If this number is high and or rapidly increasing, it is an
				// indication that your predicate is too broad, and you should
		...
		return new Firehose() {
			...
			// DIY json parsing // private final ObjectMapper omapper = new
			// ObjectMapper();
			...
			public InputRow nextRow() {
				...
				if (maxCountReached() || maxTimeReached()) {
					if (waitIfmax) {
						...
						// allow this event through, and the next hasMore() call
						// will be false
					}
				}
				if (++rowCount % 1000 == 0) {
				...
				theMap.put("htags",
						(hts.length > 0) ? Lists.transform(Arrays.asList(hts), new Function<HashtagEntity, String>() {
							@Nullable
							@Override
				...
				theMap.put("utc_offset", hasUser ? user.getUtcOffset() : -1); // resolution
																				// in
																				// seconds,
																				// -1
																				// if
